...
def check_log():
    file_name = "myapp/logs/runtime.log"
...
def check_files_creates(new_files):
    not_found = False
    if "./myapp/logging.properties" not in new_files:
        print("Missing myapp/logging.properties file")
        not_found = True
    if "./myapp/config.json" not in new_files:
        print("Missing myapp/config.json file")
        not_found = True
    if "./myapp/logs/runtime.log" not in new_files:
        print("Missing myapp/logs/runtime.log file")
        not_found = True
    if "./myapp/applications.json" not in new_files:
        print("Missing myapp/applications.json file")
        not_found = True
    if not_found:
...
def check_application_config():
    json = read_json_file("myapp/applications.json")
    checks_passed = True
...
def clean_up():
    remove_directory("myapp")


"""
Starting the runtime with a specific configName. The second time the application is no longer provided.
Is the start sucessful and all files created within the directory with the new name?
"""
if __name__ == '__main__':
    print("Launch Runtime with configName defined")
    files_at_start = get_all_files(".")
    runner = AtbashRunner()
    runner.launch_runtime(["--configName", "myapp", "demo-rest.war"])
    ...
    print("launching a second time, without pointing to war file")
    runner.launch_runtime(["--configName", "myapp"])
    ...
    clean_up()
    print("Scenario 14 testing completed successfully")
