...
"""
Start Runtime with 1 application on port 8888. After Health endpoints is ok, it checks if the application is indeed accessible.

"""
if __name__ == '__main__':
    print("Launching the Runtime with an application on different port")
    files_at_start = get_all_files(".")
    runner = AtbashRunner()
    runner.launch_runtime(["--port", "8888", "demo-rest.war"], port=8888)

    response = get_on_endpoint("http://localhost:8888/demo-rest/rest/hello/Atbash")
    ...
    print("Launching a second time without ")

    runner.launch_runtime(port=8888)  # No parameters (port is for the health check, not setting the port, are previous ones picked up?
    ...
    clean_up()
    print("Scenario 19 testing completed successfully")
