...
"""
Start Runtime with specific JMX port. Des JMX has bean with the info?

"""
if __name__ == '__main__':
    print("Launching the Runtime with specific JMX port")
    files_at_start = get_all_files(".")
    runner = AtbashRunner()
    runner.launch_runtime(jvm_params=jmx_config(), user_params=["demo-rest.war"])
    ...
    check_JMX_atbash_info(output)

    output = execute_command(
        ["java", "-jar", "jmxterm-1.0.2-uber.jar", "-l", "localhost:8765", "-i", "Atbash-applications.jmx"])
    check_JMX_atbash_applications(output)

    console = runner.end_process()

    clean_up()
    print("Scenario 20 testing completed successfully")
